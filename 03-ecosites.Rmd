...
```{r}
my.components.MLRA.reduced <- my.components.MLRA %>% site() %>% filter(compkind != "miscellaneous area" & majcompflag == "1")
...
```{r}
table(my.components.MLRA.reduced$ecosite_id) %>% as.data.frame() %>% dplyr::rename(Ecosite = Var1) %>% arrange(desc(Freq))
```
...
![](https://www.youtube.com/watch?v=9H8sz5fZaEQ)
