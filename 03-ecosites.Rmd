...
```{r}
test <- aqp::site(my.components.MLRA) %>% filter(ecosite_id %in% str_subset(my.components.MLRA@site$ecosite_id, pattern = "&")) %>% select(coiid, ecosite_id)
```


```{r}
MLRA18_ecosites <- aqp::site(my.components.MLRA) %>% dplyr::filter(ecosite_id %in% str_subset())
```
